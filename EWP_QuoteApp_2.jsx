...
export default function App() {
  ...
  const [projects, setProjects] = useState(() => {
    try { return JSON.parse(localStorage.getItem("ewp_projects") || "[]"); } catch { return []; }
  });
  ...
  // Sync rooms array when project.rooms changes
  useEffect(() => {
    const count = parseInt(project.rooms) || 1;
    setRooms(prev => {
      if (count > prev.length) return [...prev, ...Array(count - prev.length).fill(null).map((_, i) => blankRoom(prev.length + i, project.masterAdj))];
  ...
  const startNew = () => {
    const id = genId();
    setProject({ id, name: "", address: "", contactName: "", contactPhone: "", email: "", bidDate: new Date().toISOString().slice(0, 10), contractorYN: "No", contractorName: "", contractorContact: "", rooms: 1, masterAdj: 0 });
    setRooms([blankRoom(0, 0)]);
    setStep(0);
    setEditIdx(null);
    setView("new");
  };
  ...
  const saveProject = () => {
    ...
    const next = editIdx !== null
      ? projects.map((p, i) => i === editIdx ? entry : p)
      : [...projects, entry];
    setProjects(next);
    try { localStorage.setItem("ewp_projects", JSON.stringify(next)); } catch {}
      ...
      <div className="app">
        ...
        <div className="topbar">
          ...
          <div className="topbar-right">
            {view !== "dashboard" && (
              <button className="btn btn-outline btn-sm" style={{ color: "var(--rule)", borderColor: "rgba(255,255,255,0.15)" }}
        ...
        {/* STEPPER (only when building an estimate) */}
        {view === "new" && (
        ...
        <div className="main">
          ...
          {view === "new" && step === 2 && (
            <SummaryPage project={project} rooms={rooms}
              onBack={() => setStep(1)} onSave={saveProject} />
          )}
        </div>
